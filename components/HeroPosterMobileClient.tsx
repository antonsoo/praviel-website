...
type HeroPosterMobileClientProps = {
  className?: string;
};

export default function HeroPosterMobileClient({ className = "" }: HeroPosterMobileClientProps) {
  const figureClasses = [
  ...
  const intentReady = useUserIntentGate({ scrollDistance: 96, fallbackDelay: 15000 });

  useEffect(() => {
    if (shouldLoadImage || !intentReady) return;
    const cancel = scheduleIdleTask(() => setShouldLoadImage(true), { timeout: 900 });
    return cancel;
  }, [intentReady, shouldLoadImage]);
  ...
  return (
    <figure className={figureClasses}>
      {shouldLoadImage ? (
        <>
          <img
            src="/hero-poster-mobile-720.avif"
            srcSet={AVIF_SRCSET}
            sizes={SIZES}
            width={960}
            height={640}
