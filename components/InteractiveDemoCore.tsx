    ...
    >
      ...
      <div className="mx-auto max-w-5xl relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-xs text-zinc-500 uppercase tracking-wider">Selected word</p>
                  <p className={`text-2xl text-white mt-1 ${activeExcerpt.fontClass ?? "font-serif"}`}>
                    {selectedToken.text}
                  </p>
                </div>

                <div>
                  <p className="text-xs text-zinc-500 uppercase tracking-wider">Lemma</p>
                  <p className={`text-lg text-zinc-200 mt-1 ${activeExcerpt.fontClass ?? "font-serif"}`}>
                    {selectedToken.lemma}
                  </p>
                </div>

                <div>
                  <p className="text-xs text-zinc-500 uppercase tracking-wider">Definition</p>
                  <p className="text-base text-zinc-200 mt-1">{selectedToken.definition}</p>
                </div>

                <div>
                  <p className="text-xs text-zinc-500 uppercase tracking-wider">Morphology</p>
                  <p className="text-sm text-zinc-300 mt-1">{selectedToken.morphology}</p>
                </div>

                {selectedToken.notes ? (
                  <div className="rounded-lg border border-[#E8C55B]/20 bg-[#E8C55B]/5 p-3">
                    <p className="text-xs uppercase tracking-wider text-[#E8C55B]">Commentary</p>
                    <p className="text-sm text-zinc-200 mt-1">{selectedToken.notes}</p>
                  </div>
                ) : null}
              </div>
            </m.div>
        ...
        <p className="text-center text-sm text-zinc-500 mt-8">
          PRAVIEL enforces manuscript-accurate text and delivers every morphological detail cited from LSJ, Perseus, TLA, and ORACC.
        </p>
      </div>
    </section>
  );
