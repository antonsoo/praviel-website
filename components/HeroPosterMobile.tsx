...
export default function HeroPosterMobile({ className = "" }: HeroPosterMobileProps) {
  const figureClasses = [
    "relative mx-auto mb-6 block w-full max-w-xl overflow-hidden rounded-[32px] border border-white/12 bg-gradient-to-b from-[#090909] via-[#050505] to-[#010101] shadow-2xl shadow-black/60 md:hidden",
    ...
    <figure className={figureClasses}>
      ...
      <div className="relative z-10 flex h-full flex-col justify-between p-6 text-left">
        <div className="space-y-2">
          <p className="hidden sm:block text-[0.75rem] font-semibold uppercase tracking-[0.35em] text-[#E8C55B]/80">
            Fragment CXLII / Alexandria
          </p>
          <p className="hidden sm:block text-xs uppercase tracking-[0.35em] text-zinc-500">
            Manuscript Room / 240 BCE
          </p>
        </div>

        <blockquote className="space-y-2 max-w-xs sm:max-w-sm">
          <p className="text-[0.65rem] font-semibold uppercase tracking-[0.35em] text-zinc-400 sm:hidden">
            Originals only
          </p>
          <p className="hidden sm:block text-2xl font-semibold leading-snug text-white text-balance">
            "Every translation is an interpretation. To meet the author, learn their tongue."
          </p>
          <p className="hidden sm:block text-xs text-[#E8C55B]/80 uppercase tracking-[0.3em]">
